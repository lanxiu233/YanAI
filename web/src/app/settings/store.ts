...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    daily_checkin_min_quota: Number(config.daily_checkin_min_quota || 1),
    daily_checkin_max_quota: Number(config.daily_checkin_max_quota || 5),
    ...
    smtp_from_name: typeof config.smtp_from_name === "string" ? config.smtp_from_name : "颜值AI",
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        daily_checkin_min_quota: Math.max(0, Number(config.daily_checkin_min_quota) || 0),
        daily_checkin_max_quota: Math.max(
          Math.max(0, Number(config.daily_checkin_min_quota) || 0),
          Number(config.daily_checkin_max_quota) || 0,
        ),
        email_verification_enabled: Boolean(config.email_verification_enabled),
        email_domain_whitelist_enabled: Boolean(config.email_domain_whitelist_enabled),
        email_alias_restriction_enabled: Boolean(config.email_alias_restriction_enabled),
        ...
        smtp_from_name: String(config.smtp_from_name || "颜值AI").trim(),
        smtp_use_ssl: Boolean(config.smtp_use_ssl),
        smtp_use_starttls: Boolean(config.smtp_use_starttls),
        smtp_force_auth_login: Boolean(config.smtp_force_auth_login),
        linuxdo_oauth_enabled: Boolean(config.linuxdo_oauth_enabled),
