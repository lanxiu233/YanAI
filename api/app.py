...
from api.cache_headers import ASSET_CACHE_CONTROL, CachedStaticFiles, cache_control_for_asset_path
...
def create_app() -> FastAPI:
    ...
    if config.images_dir.exists():
        app.mount(
            "/images",
            CachedStaticFiles(directory=str(config.images_dir), cache_control=ASSET_CACHE_CONTROL),
            name="images",
        )
    app.mount(
        "/prompt-assets",
        CachedStaticFiles(directory=str(config.prompt_assets_dir), cache_control=ASSET_CACHE_CONTROL),
        name="prompt-assets",
    )
    ...
    async def serve_web(full_path: str):
        ...
        if asset is not None:
            return FileResponse(asset, headers={"Cache-Control": cache_control_for_asset_path(full_path)})
        if full_path.strip("/").startswith("_next/"):
            raise HTTPException(status_code=404, detail="Not Found")
        fallback = resolve_web_asset("")
        if fallback is None:
            raise HTTPException(status_code=404, detail="Not Found")
        return FileResponse(fallback, headers={"Cache-Control": cache_control_for_asset_path("")})
